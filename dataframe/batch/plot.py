...
for batch_size in batch_sizes:
    ...
    summary["batch size"].append(batch_size)
    summary["acc"].append(df["acc"].max())
    summary["f1"].append(df["f1"].max())
...
df.plot(kind='line', x='batch size', y='acc', color='red', ax=ax, title='MNIST')
df.plot(kind='line', x='batch size', y='f1', ax=ax)
