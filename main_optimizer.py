...
optimizer = "adam"
optimizer_D = torch.optim.Adam(discriminator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2), weight_decay=1e-3, amsgrad=True)
optimizer_R = torch.optim.Adam(recognizer.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2), weight_decay=1e-3, amsgrad=True)
# optimizer_D = torch.optim.SGD(discriminator.parameters(), lr=0.01, momentum=0.9)
# optimizer_R = torch.optim.SGD(recognizer.parameters(), lr=0.01, momentum=0.9)
# optimizer_D = torch.optim.NAdam(discriminator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2), weight_decay=1e-3)
# optimizer_R = torch.optim.NAdam(recognizer.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2), weight_decay=1e-3)
...
for imgs in training_iterator:
    with torch.autograd.set_detect_anomaly(True):
        ...
        if opt.model == 'pan':
            ...
            # loss2 = loss2 = 0.0001 * torch.sum(kl)
